...
class ProfessionalHomeCopy extends StatefulWidget {
  final ControllerMainProfessional controller;
  final TrackLeadsController trackLeadCntrl;

  const ProfessionalHomeCopy(
      {super.key, required this.controller, required this.trackLeadCntrl});

  @override
  State<ProfessionalHomeCopy> createState() => _ProfessionalHomeState();
}

class _ProfessionalHomeState extends State<ProfessionalHomeCopy> {
  final myActivityCntrl = Get.put(MyActivityController());

  /// Height reserved for the bottom nav bar (from ScreenMain) so content doesn't sit under it.
